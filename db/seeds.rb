...
user = User.create(email: 'afonso@lewagon.com', password: '123456')
User.create(email: 'francisca@lewagon.com', password: '123456')
User.create(email: 'cannelle@lewagon.com', password: '123456')
...
power_teleportation = Power.create(name: "Teleportation", description: "The users can teleport; moving instantaneously from one location to another without physically occupying the space in between or transfer matter (beings/objects, including themselves) or energy from one point to another without traversing the physical space between them.", user_id: user.id)
...
power_magic = Power.create(name: "Magic", description: "Users are able to utilize magic, the use of rituals, symbols, action, gestures, language, etc., to exploit supernatural forces to varying degrees, with only their skill, personal power-level, imagination/knowledge, and/or morality to define the borders. Given the flexibility of magic and their ability, its users have essentially limitless possibilities for what they can accomplish and achieve, capable of altering reality through their gifts.", user_id: user.id)
...
power_freezing = Power.create(name: "Freezing", description: "The user can reduce the kinetic energy of atoms meaning they effectively make things colder, ranging from slightly chilly levels to absolute zero.", user_id: user.id)
...
power_persuasion = Power.create(name: "Persuasion", description: "The user can compel people by speaking, the victims are unable to disobey; the seemingly cogent commandment is far too compelling. At a high level, users can persuade people into hurting/killing themselves or even flip around sense of logic, but can never cause victims to achieve what they are not capable of (ex. Flight).", user_id: user.id)
...
power_gravity = Power.create(name: "Gravity Manipulation", description: "Users can create, shape, move, control, interact and manipulate gravity, a natural phenomenon which all things with mass or energy — including planets, stars, galaxies, and light — attract (or gravitate toward) one another.", user_id: user.id)
...
power_energy = Power.create(name: "Energy Blast", description: "The user can release energy over a specific target area causing great damage and/or delivering great shock waves of pure force.", user_id: user.id)
